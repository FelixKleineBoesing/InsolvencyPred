...
from evaluation import  get_f1_score, get_accuracy, get_auc, get_precision, get_recall
...
def main():
    ...
    for i, df in enumerate(dfs):
        ...
        probs = cv.run(data=df, label=label, model=model,
                       preprocessors=[MeanReplacement(), Standardizer(), ReSampler("down"), PCA(0.98)])
        ...
        print("Recall, Baseline: {}, LogisticRegression: {}".format(get_recall(np.random.choice([0, 1], size=len(label)), label),
                                                         get_recall(preds, label)))
        print("Precision, Baseline: {}, LogisticRegression: {}".format(get_precision(np.random.choice([0, 1], size=len(label)), label),
                                                            get_precision(preds, label)))
    ...
    for i, df in enumerate(dfs):
        ...
        model = XGBoost(val_share=0.2, n_rounds=10, lambda_=0,
                        additional_booster_params={"params": {"max_depth": 4, "subsample": 0.7,
                                                              "colsample_bytree": 0.7, "scale_pos_weight": 13}})
        ...
        print("Acc, Baseline: {}, Xgboost: {}".format(get_accuracy([0 for _ in range(len(label))], label),
                                                      get_accuracy(preds, label)))
        print("AUC, Baseline: {}, Xgboost: {}".format(get_auc(np.random.rand(len(label)), label),
                                                      get_auc(probs_xgb[i], label)))
        print("Recall, Baseline: {}, Xgboost: {}".format(get_recall(np.random.choice([0, 1], size=len(label)), label),
                                                         get_recall(preds, label)))
        print("Precision, Baseline: {}, Xgboost: {}".format(get_precision(np.random.choice([0, 1], size=len(label)), label),
                                                            get_precision(preds, label)))
